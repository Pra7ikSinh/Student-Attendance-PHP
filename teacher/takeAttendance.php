...
$selectedClassId = isset($_POST['selectedClass']) ? $_POST['selectedClass'] : null;
$selectedSectionId = isset($_POST['selectedSection']) ? $_POST['selectedSection'] : null;
$selectedSubjectId = isset($_POST['selectedSubject']) ? $_POST['selectedSubject'] : null;

$query = "SELECT tblclass.className, tblclassarms.classArmName
          FROM tblteacher
          INNER JOIN tblclass ON tblclass.Id = '$selectedClassId'
          INNER JOIN tblclassarms ON tblclassarms.Id = '$selectedSectionId'
...
$qurty = mysqli_query($conn, "SELECT * FROM tblattendance WHERE classId = '$selectedClassId' AND classArmId = '$selectedSectionId' AND dateTimeTaken='$dateTaken'");
$count = mysqli_num_rows($qurty);

if ($count == 0) {
    // If Record does not exist, insert the new record
    $qus = mysqli_query($conn, "SELECT * FROM tblstudents WHERE classId = '$selectedClassId' AND classArmId = '$selectedSectionId'");
    $ros = $qus->fetch_assoc();

    while ($ros = $qus->fetch_assoc()) {
        $enrollmentNo = $ros["Enrollment_no"];
        $qquery = mysqli_query($conn, "INSERT INTO tblattendance(Enrollment_no,classId,classArmId,sessionTermId,status,dateTimeTaken)
                                       VALUES ('$enrollmentNo','$selectedClassId','$selectedSectionId','$sessionTermId','0','$dateTaken')");
    }
}
...
if (isset($_POST['save'])) {
    ...
    if (isset($_POST['Enrollment_no']) && isset($_POST['check'])) {
        ...
        $qurty = mysqli_query($conn, "SELECT * FROM tblattendance WHERE classId = '$selectedClassId' AND classArmId = '$selectedSectionId' AND dateTimeTaken='$dateTaken' AND status = '1'");
        $count = mysqli_num_rows($qurty);
                  ...
                  <div class="col-lg-12">
                    <div class="card mb-4">
                      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">All Student in (<?php echo $selectedClassId . ' - ' . $selectedSectionId; ?>) Class</h6>
...
$query = "SELECT tblstudents.Id,tblstudents.Enrollment_no,tblclass.className,tblclass.Id As classId,tblclassarms.classArmName,tblclassarms.Id AS classArmId,tblstudents.firstName,
                      ...
                      where tblstudents.classId = '$selectedClassId' and tblstudents.classArmId = '$selectedSectionId'";
